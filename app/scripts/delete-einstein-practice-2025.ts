// Delete Einstein 2025 practiceMatches docs.
//
// Usage:
//   npx tsx app/scripts/delete-einstein-practice-2025.ts          (dry run)
//   npx tsx app/scripts/delete-einstein-practice-2025.ts --apply  (delete docs)
//
// Optional:
//   --event=2025cmptx
...
async function run() {
  ...
  for (const doc of targets.slice(0, 20)) {
    ...
    const teamNumber = readString(doc.fields?.teamNumber);
    console.log(`  - ${id} | ${matchKey} | ${alliance} | team=${teamNumber || "-"}`);
  }
