// Update practiceMatches difficulty based on new score thresholds.
//
// Usage:
//   npx tsx app/scripts/migrate-practice-difficulty-thresholds.ts          (dry run)
//   npx tsx app/scripts/migrate-practice-difficulty-thresholds.ts --apply  (write updates)
//
// Optional:
//   --event=2026tuis
...
function readNumber(field: FirestoreField | undefined): number | null {
...
function readNumberFromMap(field: FirestoreField | undefined, key: string): number | null {
  const fields = field?.mapValue?.fields;
  if (!fields) return null;
  return readNumber(fields[key]);
}
...
function readScore(fields: Record<string, FirestoreField>): number | null {
  const candidates = [
    readNumber(fields.officialScore),
    readNumberFromMap(fields.officialData, "score"),
    readNumber(fields.actualScore),
    readNumber(fields.allianceScore),
  ];
...
async function patchDifficulty(
  docName: string,
  nextDifficulty: "easy" | "medium" | "hard",
  authHeaders: Record<string, string>
) {
  const params = new URLSearchParams();
  params.append("updateMask.fieldPaths", "difficulty");
  ...
  const body = {
    fields: {
      difficulty: { stringValue: nextDifficulty },
      migratedAt: { integerValue: String(Date.now()) },
      migrationVersion: { stringValue: "practice-difficulty-thresholds-v1" },
    },
  };
...
async function run() {
  ...
  for (const doc of docs) {
    ...
    const score = readScore(fields);
    if (score === null) continue;
    const nextDifficulty = scoreToDifficulty(score);
    const currentDifficulty = readString(fields.difficulty).toLowerCase();
    if (currentDifficulty !== nextDifficulty) {
      candidates.push({
        name: doc.name,
        id: doc.name.split("/").pop() || "",
        eventKey: docEventKey,
        matchKey: readString(fields.matchKey),
        fromDifficulty: currentDifficulty || "(missing)",
  ...
  for (const row of candidates.slice(0, 30)) {
    console.log(`  - ${row.id} | ${row.eventKey} | ${row.matchKey} | ${row.fromDifficulty} -> ${row.toDifficulty}`);
  }
  ...
  for (const row of candidates) {
    await patchDifficulty(row.name, row.toDifficulty, authHeaders);
    updated += 1;
