...
function MatchBox({
  ...
  return (
    <button
      ...
      className={`relative w-[120px] min-h-[62px] text-xs rounded border text-left bg-white border-t border-b border-l border-r ${borderColors[match.status]} hover:bg-gray-50`}
    >
      <div className={`absolute top-0.5 right-0.5 text-[10px] px-1 py-0.5 rounded-full text-white ${badgeColors[match.status]}`}>
      ...
      <div className="pt-1.5 pb-1 px-1.5">
        ...
        <div className="mt-1.5 border-t border-gray-200 pt-1">
          {(() => {
            ...
            return <div className="text-[10px] text-gray-600 text-center">{timeString}</div>;
          })()}
        </div>
      </div>
    </button>
    ...
    <div className="relative w-full flex justify-center py-6 overflow-x-auto">
      <div style={{ width: totalWidth }}>
        ...
        <div className="relative" style={{ width: totalWidth, height }}>
          ...
          <div className="absolute" style={{ left: c0, top: r1_1 }}><MatchBox match={{ id: 1, label: "Match 1", status: "completed", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c0, top: r1_2 }}><MatchBox match={{ id: 2, label: "Match 2", status: "completed", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c0, top: r1_3 }}><MatchBox match={{ id: 3, label: "Match 3", status: "completed", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c0, top: r1_4 }}><MatchBox match={{ id: 4, label: "Match 4", status: "completed", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c1, top: r2_7 }}><MatchBox match={{ id: 7, label: "Match 7", status: "completed", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c1, top: r2_8 }}><MatchBox match={{ id: 8, label: "Match 8", status: "next", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c1, top: lower_5 }}><MatchBox match={{ id: 5, label: "Match 5", status: "completed", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c1, top: lower_6 }}><MatchBox match={{ id: 6, label: "Match 6", status: "completed", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c2, top: lower_9 }}><MatchBox match={{ id: 9, label: "Match 9", status: "completed", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c2, top: lower_10 }}><MatchBox match={{ id: 10, label: "Match 10", status: "completed", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c3, top: r3_11 }}><MatchBox match={{ id: 11, label: "Match 11", status: "upcoming", bracket: "upper" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c3, top: lower_12 }}><MatchBox match={{ id: 12, label: "Match 12", status: "upcoming", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c4, top: y13 }}><MatchBox match={{ id: 13, label: "Match 13", status: "upcoming", bracket: "lower" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
          <div className="absolute" style={{ left: c5, top: yFinals }}><MatchBox match={{ id: 14, label: "FINALS", status: "upcoming" }} setSelectedMatch={setSelectedMatch} setModalOpen={setModalOpen} /></div>
        </div>
      </div>
    </div>
  );
...
function ScoutFormContent() {
  ...
  const [selectedMatch, setSelectedMatch] = useState<{ 
    id: number; 
    type?: "qualification" | "practice" | "finals"; 
    bracket?: "upper" | "lower" 
  }>({ id: 23, type: "qualification" });
  ...
  // EVENT CHECK - Redirect if no event active
  useEffect(() => {
    if (!isEventActive()) {
      alert('No active event right now. Please use Practice Scouting to train!');
      router.push('/practice-scouting');
    }
  }, [router]);
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row justify-center">
          <div className="flex-1 p-4 space-y-6 max-w-3xl">
            {/* MATCH SELECTOR */}
            <div className="bg-white rounded-xl shadow p-4 border-l-4" style={{ borderColor: "#c42221" }}>
              <div className="flex items-center gap-3">
                <span className="text-lg font-semibold">Assigned Match:</span>
                <span className="text-lg font-semibold" style={{ color: "#c42221" }}>{getMatchDisplay()}</span>
                <button onClick={() => { setModalStep("type"); setModalOpen(true); }} className="px-2 py-0.5 text-xs rounded text-white" style={{ backgroundColor: "#c42221" }}>Fix</button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow p-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={formData.scoutName} disabled className="w-full border rounded p-2 bg-gray-100 text-gray-600" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Team Number</label>
                  <select value={formData.teamNumber} onChange={(e) => setFormData({ ...formData, teamNumber: e.target.value })} className="w-full border rounded p-2">
            ...
            {/* SUBMIT */}
            <div className="bg-white rounded-xl shadow p-4">
              <button onClick={handleSubmit} className="w-full py-3 rounded text-white font-semibold" style={{ backgroundColor: "#c42221" }}>
          ...
          <div className="hidden md:block w-80 p-4">
            <div className="bg-white rounded-xl shadow p-4 sticky top-4 flex flex-col" style={{ height: 'calc(100vh - 2rem)' }}>
              <h2 className="text-xl font-semibold mb-2" style={{ color: "#c42221" }}>Notes</h2>
              <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="flex-1 border rounded p-2 resize-none" placeholder="Write notes here..." />
            </div>
          </div>

          {/* MOBILE NOTES */}
          <div className="md:hidden fixed right-0 top-1/2 transform -translate-y-1/2 z-40">
            <button onClick={() => setNotesOpen(!notesOpen)} className="px-2 py-4 rounded-l-xl text-white" style={{ backgroundColor: "#c42221" }}>
              {notesOpen ? "→" : "←"}
            </button>
            {notesOpen && (
              <div className="fixed right-0 top-0 h-full w-64 bg-white shadow-xl p-4 z-50">
                <h2 className="text-xl font-semibold mb-2" style={{ color: "#c42221" }}>Notes</h2>
                <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="w-full h-[85%] border rounded p-2 resize-none" placeholder="Write notes here..." />
              </div>
            )}
          </div>

          {/* MODAL */}
          <Modal open={modalOpen} onClose={() => setModalOpen(false)} step={modalStep}>
            {modalStep === "type" && (
              <>
                <h2 className="text-xl font-semibold mb-4" style={{ color: "#c42221" }}>Select Match Type</h2>
                <div className="space-y-3">
                  <button onClick={() => setModalStep("practice")} className="w-full py-2 rounded text-white" style={{ backgroundColor: "#c42221" }}>Practice</button>
                  <button onClick={() => setModalStep("qualification")} className="w-full py-2 rounded text-white" style={{ backgroundColor: "#c42221" }}>Qualification</button>
                  <button onClick={() => setModalStep("finals")} className="w-full py-2 rounded text-white" style={{ backgroundColor: "#c42221" }}>Finals</button>
                </div>
              </>
            )}
            {modalStep === "practice" && (
              <>
                <h2 className="text-xl font-semibold mb-4" style={{ color: "#c42221" }}>Practice Match</h2>
                <p className="text-gray-600 mb-4">Enter match number manually.</p>
                <input type="number" placeholder="Practice Match #" className="w-full border rounded p-2 mb-4" id="practiceMatchInput" />
                <button className="w-full py-2 rounded text-white" style={{ backgroundColor: "#c42221" }} onClick={() => {
            ...
            {modalStep === "qualification" && (
              <>
                <h2 className="text-xl font-semibold mb-4" style={{ color: "#c42221" }}>Qualification Matches</h2>
                {(() => {
                  ...
                  const schedule = Array.from({ length: totalMatches }, (_, i) => {
                    const matchNum = i + 1;
                    const matchTime = new Date(baseTime.getTime() + i * 7 * 60000);
                  ...
                  const borderColors: Record<MatchStatus, string> = { completed: "border-green-500", next: "border-yellow-500", upcoming: "border-red-500" };
                  const badgeColors: Record<MatchStatus, string> = { completed: "bg-green-500", next: "bg-yellow-500", upcoming: "bg-red-500" };
                  const badgeEmojis: Record<MatchStatus, string> = { completed: "✔️", next: "⏳", upcoming: "❌" };
                  return (
                    <div className="grid grid-cols-3 gap-2 max-h-[60vh] overflow-y-auto pr-1">
                      {schedule.map(({ matchNum, timeString, status }) => (
                        <button key={matchNum} onClick={() => { setSelectedMatch({ id: matchNum, type: "qualification" }); setModalOpen(false); }} className={`relative p-2 rounded-lg border text-left ${borderColors[status]} hover:bg-gray-50`}>
                          <div className={`absolute top-0.5 left-0.5 text-[10px] px-1 py-0.5 rounded-full text-white ${badgeColors[status]}`}>{badgeEmojis[status]}</div>
                          <div className="mt-3">
                            <div className="font-semibold text-sm">Qualification {matchNum}</div>
                ...
                <button className="w-full mt-4 py-2 rounded text-white" style={{ backgroundColor: "#c42221" }} onClick={() => setModalOpen(false)}>Close</button>
              </>
            )}
            {modalStep === "finals" && <FinalsBracket setSelectedMatch={handleMatchSelect} setModalOpen={setModalOpen} />}
          </Modal>
        </div>
      </div>
    </div>
