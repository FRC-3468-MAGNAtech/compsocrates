...
import { EVENTS, isWithinEventDates } from "@/app/utils/eventDates";
...
// Helper function to format match display - FIXED FOR FINALS
function formatMatchDisplay(entry: Entry): string {
  ...
  if (matchType === "practice") {
    ...
    return `F${matchNum}`;  // ← FIX: Now shows F instead of Q
  } else {
    return `Q${matchNum}`;
  }
}
...
function AnalyticsPageContent() {
  ...
  const [showPractice, setShowPractice] = useState(false); // NEW: Practice toggle
  ...
  // UPDATED: Event filtering with date checking
  const filteredData = rawData.filter(entry => {
    // Filter by practice toggle
    if (!showPractice && entry.matchType === "practice") {
      return false;
    }
    
    // Filter by event
    if (selectedEvent !== "all" && selectedEvent !== "app-testing") {
      if (!entry.submittedAt) return false;
      return isWithinEventDates(entry.submittedAt, selectedEvent);
    }
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {activeView === "raw-data" && (
          <>
            {/* UPDATED HEADER WITH EVENT FILTER AND PRACTICE TOGGLE */}
            <div className="p-4 bg-white border-b flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                >
                  ...
                  <option value="rocketCity">Rocket City Regional</option>
