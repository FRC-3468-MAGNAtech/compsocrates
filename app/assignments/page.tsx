...
function buildFirstPracticeSeeds(eventKey: string, matches: Awaited<ReturnType<typeof fetchFirstSchedule>>): FirstPracticeSeed[] {
  ...
  return matches.flatMap((match) => {
    ...
    if (red.length >= 3) {
      seeds.push({
        id: `first_${safeEvent}_practice_${matchNumber}_red`,
        matchNumber,
        matchKey,
        alliance: "red",
        teams: red.slice(0, 3),
    ...
    if (blue.length >= 3) {
      seeds.push({
        id: `first_${safeEvent}_practice_${matchNumber}_blue`,
        matchNumber,
        matchKey,
        alliance: "blue",
...
function AssignmentsContent() {
  ...
  useEffect(() => {
    ...
    void ensurePracticeMatchesForEvent(selectedEvent);
  }, [selectedEvent]);
...
function buildMatchScoutOrder(
  ...
  async function selectMatchType(type: "practice" | "qualification" | "finals") {
    ...
    setSelectedTeamNumber("");
    if (type === "practice") {
      const available = practiceMatchChoices;
      if (available.length === 0) {
        await ensurePracticeMatchesForEvent(selectedEvent);
      }
    }
  }
  ...
  const practiceScheduleMatches = useMemo(
    () => (selectedEvent ? practiceScheduleMatchesByEvent[selectedEvent] || [] : []),
    [practiceScheduleMatchesByEvent, selectedEvent]
  );
  ...
  useEffect(() => {
    ...
    if (!selectedEvent) return;
    void loadPracticeScheduleEvent(selectedEvent);
  }, [scheduleView, selectedEvent]);
